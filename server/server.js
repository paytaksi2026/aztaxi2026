...
io.on("connection",socket=>{
 ...
 socket.on("ride-finish",data=>{
   ...
   if(!balances[data.driver]) balances[data.driver]=0

   balances[data.driver]+=data.price

 })

 socket.on("rating",data=>{

   if(!ratings[data.driver]) ratings[data.driver]=[]

   ratings[data.driver].push(data.rate)
...
server.listen(process.env.PORT||3000,()=>{
 console.log("AzTaxi PRO server running")
})
